  ...
  api.get("movie/").then(res => {
    ...
    api.get("genre/").then(res => {
      ...
      genres.forEach(genre => {
        ...
        if (collection.movies.length > 0) payload.collections.push(collection);
      });
...
export const editGenre = (movieId, genreId) => dispatch => {
  ...
  api
    .get(`movie/${movieId}/genres/`)
    .then(res => {
      api
        .patch(`movie_genre/${res.data[0].id}/`, data)
        .then(res => {
...
export const addReview = (movieId, review) => dispatch => {
  api
    ...
    .then(res => {
      ...
      api
        .post(`movie/${movieId}/add_review/`, data)
        .then(res => {
          console.log("Success: ", res.data);
...
export const addMovie = (movieDetails, genreId, history) => dispatch => {
  api
    ...
    .then(res => {
      ...
      api
        .post(`/movie/${movie.id}/add_genre/`, data)
        .then(res => {
          console.log(res.data);
          history.push("/");
        })
...
export const addOrder = order => dispatch => {
  api.post("/order/", order).catch(err =>
    dispatch({
      type: GET_ERRORS,
