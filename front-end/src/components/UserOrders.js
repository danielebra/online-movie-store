...
class UserOrders extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.movies.orders) {
      ...
      orders = nextProps.movies.orders.filter(
        order => order.user === JSON.parse(localStorage.user).id
      );
  ...
  filtering = (orders, filterDate) => {
    return orders.filter(
      order =>
        this.reformatDate(new Date(order.date.toLocaleString())) ===
        this.reformatDate(filterDate)
    );
  };
            ...
            <td>
              {this.state.isEditing[index] ? (
                ...
                <Fragment>
                  <td>
                    <i
                      onClick={() => this.switchToEditingMode(index)}
                      className="material-icons pointer"
                    >
                      edit
                    </i>
                ...
                <table className="table bordered highlight centered responsive-table management-table">
                  <thead>
                    <tr>
                      ...
                      <th scope="col">
                        <i class="material-icons previx">settings</i>
                      </th>
                    </tr>
                  </thead>
                  <tbody>{this.orderTable(orders)}</tbody>
                </table>
              </form>
...
export default connect(
  mapStateToProps,
  { getOrders, getMovieById, deleteOrder, updateOrder }
