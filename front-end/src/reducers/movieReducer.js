...
export default function(state = initialState, action) {
  ...
  if (localStorage.wishlist) state.wishList = JSON.parse(localStorage.wishlist);
  ...
  switch (action.type) {
    ...
    case SEARCH_MOVIES_ADMIN:
      ...
      movies = state.moviesList.filter(movie =>
        movie.genre[0].toLowerCase().includes(action.payload.toLowerCase())
      );
    ...
    case SEARCH_MOVIES:
      ...
      movies = state.collections.filter(
        movie => movie.genre.toLowerCase() == action.payload
      );
      ...
      if (movies.length == 0) {
        state.moviesList.forEach(movie => {
          if (movie.title.toLowerCase().includes(action.payload)) {
            ...
            if (!added) movies.push({ genre: movieGenre, movies: [movie] });
          }
        });
      }
